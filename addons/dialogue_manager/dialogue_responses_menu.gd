...
class_name DialogueResponsesMenu extends HBoxContainer
...
func _focus_next():
	...
	if index == -1 or index == items.size() - 1:
		items[0].grab_focus()
	else:
		items[index + 1].grab_focus()
...
func _on_response_gui_input(event: InputEvent, item: Control, response) -> void:
	...
	elif event.is_action_pressed(&"menu" if next_action.is_empty() else next_action) and item in get_menu_items():
		get_viewport().set_input_as_handled()
		response_selected.emit(response)
