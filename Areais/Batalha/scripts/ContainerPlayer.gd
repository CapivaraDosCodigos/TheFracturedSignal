...
@onready var actions: Array[VBoxContainerButton] = [ %ATK, %ITM, %EXE, %DEF, %ACT ]
...
func _process(_delta: float) -> void:
	if Manager.PlayersAtuais.has(player):
		...
		_atualizar_acoes_bloqueadas(playerProcess)
...
func _atualizar_acoes_bloqueadas(p: PlayerData) -> void:
	for action in actions:
		if p.blocked_actions.has(action.name):
			action.ButtonContainer.modulate = Color(0.5, 0.5, 0.5)
		else:
...
func get_current_action_name() -> String:
	if current_index >= 0 and current_index < actions.size():
		if Manager.get_Player(player).blocked_actions.has(actions[current_index].name):
			return "Error"
